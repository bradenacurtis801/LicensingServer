...
# Load your server's public key (from file or environment variable)
with open("license_server_public.pem") as f:
    public_key = f.read()
...
with open("license_server_public.pem") as f:
    public_key = f.read()
client = LicenseClient("http://localhost:8999", "MyApp", "1.0.0", public_key)
...
```python
LicenseClient(server_url: str, app_name: str, app_version: str, public_key: str)
```
...
- `public_key`: PEM-formatted RSA public key for verifying server responses (obtain from your license server administrator)
...
with open("license_server_public.pem") as f:
    public_key = f.read()
client = LicenseClient("http://localhost:8999", "WebApp", "1.0.0", public_key)
feature_manager = FeatureManager(client)
...
class MyApp:
    def __init__(self):
        with open("license_server_public.pem") as f:
            public_key = f.read()
        self.client = LicenseClient("http://localhost:8999", "DesktopApp", "1.0.0", public_key)
...
def main():
    ...
    with open("license_server_public.pem") as f:
        public_key = f.read()
    client = LicenseClient("http://localhost:8999", "CLIApp", "1.0.0", public_key)
    feature_manager = FeatureManager(client)
